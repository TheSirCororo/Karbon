...
class FlexibleStorage(
    ...
    val data: LongArray = LongArray((4096 + (64 / bitsPerEntry) - 1) / (64 / bitsPerEntry))
) {

    private val size = data.size * 64 / bitsPerEntry
    private val maxEntryValue = (1L shl bitsPerEntry) - 1
    private val valuesPerLong = (64 / bitsPerEntry).toChar()
    private val magicIndex = 3 * (valuesPerLong.toInt() - 1)
    private val divideMultiply = Integer.toUnsignedLong(MAGIC_VALUES[magicIndex])
    private val divideAdd = Integer.toUnsignedLong(MAGIC_VALUES[magicIndex + 1])
    private val divideShift = Integer.toUnsignedLong(MAGIC_VALUES[magicIndex + 2])
    ...
    fun transferData(newBitsPerEntry: Int, valueGetter: (Int) -> Int): FlexibleStorage {
        val newStorage = FlexibleStorage(newBitsPerEntry)
        for (i in 0..4095) {
            newStorage[i] = valueGetter.invoke(i)
        }
        return newStorage
    }
